...
class PlayState extends MusicBeatState
{
	...
	var noteTime1:Array<Float> = [];
	var noteTime2:Array<Float> = [];
	...
	var addBattle:Bool = true;

	override public function create()
	{
		//DELTARUNE SAVE COUNT TIMER
		new FlxTimer().start(1, tmr -> {DeltaSaveControl.gameTime ++;}, 0);
		...
		{
            case 'ralseiCastle':
				var floor = new BGSprite('delta/bg/rCastle/floor', -50);
				floor.scale.set(1.4, 1.4);
                add(floor);
				
                var top = new BGSprite('delta/bg/rCastle/top', -50);
				top.scale.set(1.5, 1.35);
                add(top);
			case 'field':
				var sky = new BGSprite('delta/bg/field/floor', -550, -100);
                add(sky);

				var backT = new BGSprite('delta/bg/field/Back Trees', -500, -100, 1, 1, ['back tree'], true);
				backT.dance();
                add(backT);

				var floor = new BGSprite('delta/bg/field/floor', -500, -100);
                add(floor);

				var frontT = new BGSprite('delta/bg/field/Front Trees', -500, -100, 1, 1, ['Top tree'], true);
				frontT.dance();
                add(frontT);
			case 'shop':
				var bg = new BGSprite('delta/bg/shop/bg', -800, -300);
				bg.scale.set(.65, .65);
				add(bg);
			case 'school':
				var bg = new BGSprite('delta/bg/school/bg', -650, -290);
				bg.scale.set(.65, .65);
				add(bg);
			case 'topchef':
				var sky = new BGSprite('delta/bg/topchef/bg', -550, -100);
                add(sky);

				var floor = new BGSprite('delta/bg/topchef/floor', -550, -100);
                add(floor);

				var trees = new BGSprite('delta/bg/topchef/trees', -500, -100);
                add(trees);
		}
...
/* 		battle = new Battle();
		...
		#end
		var dadNoteTex:String = 'NOTE_assets';
		var bfNoteTex:String = 'NOTE_assets';

		switch(dad.curCharacter){
			//ralseis notes are legit the default ones
			//case 'cloakralsei' | 'opralsei': dadNoteTex = 'ralseiNOTE_assets';
			case 'opsusie' | 'lightsusie': dadNoteTex = 'susieNOTE_assets';
			case 'seam': dadNoteTex = 'seamNOTE_assets';
			case 'lancer': dadNoteTex = 'lancerNOTE_assets';
		}
		switch(boyfriend.curCharacter){
			case 'kris' | 'krisFall' | 'lightkris': bfNoteTex = 'krisNOTE_assets';
			case 'opsusie' | 'lightsusie' | 'susie': bfNoteTex = 'susieNOTE_assets';
		}

		//LOADS PLAYER/OPPONENT CUSTOME NOTE SKINS!!
		changeOpponentArrows(dadNoteTex);
		changePlayerArrows(bfNoteTex);
	...
	public function moveCamera(isDad:Bool) {
		if(isDad) {
			...
			switch (curStage)
			{
				...
				case 'shop':
					camFollow.x = dad.getMidpoint().x + 285;
...
/* 		if(isDad && addBattle && battle != null){
			FlxTween.tween(battle, {alpha: 1}, .25, {onComplete: twn ->{
				battle = new Battle();
				battle.cameras = [camHUD];
				battle.screenCenter();
				add(battle);
		...
		else if (!isDad && !addBattle && battle != null){
			FlxTween.tween(battle, {alpha: 0}, .25, {onComplete: twn ->{
				remove(battle);
			}});
			addBattle = true;
		} */
	...
	function nextSong(songName:String) {
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				switch(curSong){
					//case 'susie': MusicBeatState.switchState(new DeltaPlayState('hallAfter'));
	...
	function changeOpponentArrows(texture:String)
		{
			...
			notes.forEach(note -> {
				if(!note.mustPress) note.texture = texture;
			});
			for (i in 0...unspawnNotes.length) {
				if(!unspawnNotes[i].mustPress) unspawnNotes[i].texture = texture;
			}
		}
	function changePlayerArrows(texture:String)
		{
